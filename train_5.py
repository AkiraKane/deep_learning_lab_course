...
for i in range(ori_size):
    ...
    transformed = t.process_mirror(image, n=2)
    train_x = np.append(train_x, transformed, axis=0)
    train_y = np.append(train_y, [label] * len(transformed), axis=0)

print("new train_x: " + str(train_x.shape))
print("new train_y: " + str(train_y.shape))
...
results = net.train(train_x, train_y, Xvalid=valid_x,
